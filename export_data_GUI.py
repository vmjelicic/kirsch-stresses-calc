...
class ExportDataFrame(ttk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        ttk.Label(
            self, text="Variables to export (use comma and space as separator ', ')"
        ...
        self.folder_root_button = ttk.Button(
            self, text="Open", command=self.select_folder_path
        )
        ...
        self.export_button = ttk.Button(
            self, text="Export data", command=self.export_data
        )  # además cuando termine de exportar debe invocar un pop-up
    ...
    def active_select_variable(self):
        if self.select_variable.get():
            ...
            self.variable_to_export_entry = ttk.Entry(self, width=30)
            self.variable_to_export_entry.grid(
    ...
    def export_data(self):
        ...
        if self.select_variable.get():
            boundarybox.boundary_df[variables_to_export].to_csv(
