...
class Application(ttk.Frame):
    def __init__(self, main_window):
        ...
        self.style_notebook.configure(
            "TNotebook", foreground="black", background="gray94"
        )
    ...
    def dark_mode(self):
        if self.dark_mode_var.get():
            self.style_notebook.configure(
                "TNotebook", foreground="white", background="gray20"
            )
            self.style_frames.configure(
                "TFrame", foreground="white", background="gray20"
            )
            self.style_labels.configure(
                "TLabel", foreground="white", background="gray20"
            )
            self.style_entries.configure(
                "TEntry", foreground="black", background="gray20"
            )
            ...
            self.style_frames.configure(
                "TFrame", foreground="black", background="gray94"
            )
            self.style_labels.configure(
                "TLabel", foreground="black", background="gray94"
            )
            self.style_entries.configure(
                "TEntry", foreground="black", background="gray94"
            )
            self.style_checkbuttons.configure("TCheckbutton", background="gray94")
            self.style_text.configure("TText", foreground="black", background="gray94")
