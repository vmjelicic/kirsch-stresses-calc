...
class VisualizationFrame(ttk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        # Panel de opciones
        self.discretization_button = ttk.Button(
            self.OptionsFrame,
            text="Discretization",
            command=self.discretization,
            width=12,
        )
        self.discretization_button.grid(row=0, column=0, sticky="n", pady=5)

        self.calculate_button = ttk.Button(
            self.OptionsFrame, text="Calculate", command=self.calculate, width=12
        )
        self.calculate_button.grid(row=1, column=0, sticky="n", pady=5)

        ttk.Label(self.OptionsFrame, text="Select variable to show").grid(
        ...
        self.variable_optionmenu = ttk.OptionMenu(
            self.OptionsFrame, self.selected_variable, *self.option_list
        )
        ...
        self.plot_options_button = ttk.Checkbutton(
            self.OptionsFrame,
            command=self.active_plot_options,
            variable=self.plot_options,
            onvalue=1,
            offvalue=0,
        )
        ...
        self.select_plot_button = ttk.Button(
            self.OptionsFrame, text="Generate Plot", command=self.select_plot, width=12
        )
        ...
        self.progress_bar = ttk.Progressbar(
            self.OptionsFrame, orient=tk.HORIZONTAL, length=100, mode="determinate"
        )
    ...
    def matplot_canvas(
            ...
            for geometry in geometries:
                if geometry.__class__.__name__ == "Geometry_circle":
                    ...
                    y_geom_u = [
                        (geometry.height / 2)
                        * np.sqrt(
                            1 - ((x - geometry.x_centre) / (geometry.width / 2)) ** 2
                        )
                        + geometry.y_centre
                        for x in x_geom
                    ]
        ...
        if (min_value is not None) and (max_value is not None):
            f.colorbar(graph, label=label_plot, pad=0.05).mappable.set_clim(
                min_value, max_value
            )
    ...
    def discretization(self):
        ...
        from load_parameters_GUI import (
            boundarybox,
            geometries,
            P,
            Q,
        ...
        self.graph = self.matplot_canvas(
            x_plot=x_point, y_plot=y_point, geometries=geometries
        )
        self.toolbar = NavigationToolbar2Tk(self.graph, self.PlotFrame)
        self.toolbar.update()
        self.graph.get_tk_widget().pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)
