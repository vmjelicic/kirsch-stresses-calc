...
class LoadParametersFrame(ttk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.save_box_button = ttk.Button(
            self.BoxFrame, text="Save", command=self.save_boundarybox
        )
        ...
        for i in range(len(self.geometry_circle_labels)):
            ttk.Label(
                self.GeometryCircleFrame, text=self.geometry_circle_labels[i], width=10
            ).grid(row=self.geometry_circle_labels_rows[i], column=0, sticky="e")
        ...
        self.save_geometry_circle_button = ttk.Button(
            self.GeometryCircleFrame, text="Save", command=self.save_geometry_circle
        )
        ...
        for i in range(len(self.geometry_ellipse_labels)):
            ttk.Label(
                self.GeometryEllipseFrame,
                text=self.geometry_ellipse_labels[i],
                width=10,
            ).grid(row=self.geometry_ellipse_labels_rows[i], column=0, sticky="e")
        ...
        self.save_geometry_ellipse_button = ttk.Button(
            self.GeometryEllipseFrame, text="Save", command=self.save_geometry_ellipse
        )
        ...
        for i in range(len(self.stresses_labels)):
            ttk.Label(self.StressesFrame, text=self.stresses_labels[i], width=10).grid(
                row=self.stresses_labels_rows[i], column=0, sticky="e"
            )
        ...
        self.save_stresses_button = ttk.Button(
            self.StressesFrame, text="Save", command=self.save_stresses
        )
        self.save_stresses_button.grid(row=6, column=1)

        self.strength_option = tk.IntVar()

        self.strength_button = ttk.Checkbutton(
            self.StressesFrame,
            command=self.active_strength_inputs,
            variable=self.strength_option,
            onvalue=1,
            offvalue=0,
        )
        ...
        self.saved_parameters_text.config(
            width=70, height=17, padx=10, pady=5, state="disabled"
        )
        ...
        self.OptionsNotebook.add(
            self.GeometryCircleFrame, text="Geometry Circle", padding=10
        )
        self.OptionsNotebook.add(
            self.GeometryEllipseFrame, text="Geometry Ellipse", padding=10
        )
        self.OptionsNotebook.add(self.StressesFrame, text="Stresses", padding=10)

        # Modificaciones de historial de frame izquierdo
        self.delete_parameter_button = ttk.Button(
            self.RightFrame, text="Delete Parameter", width=15
        )  # command=
        self.delete_parameter_button.grid(row=2, column=0, sticky="s", pady=5)
        self.clear_history_button = ttk.Button(
            self.RightFrame, text="Clear History", command=self.clear_history, width=15
        )
    ...
    def save_boundarybox(self):
        ...
        self.saved_parameters_text.insert(
            tk.END,
            "BoundaryBox(dim_x={}, dim_y={}, sep_x={}, sep_y={})\n".format(
                dim_x, dim_y, sep_x, sep_y
            ),
        )
    ...
    def save_geometry_circle(self):
        ...
        new_geometry_circle = Geometry_circle(
            name=name, x_centre=x_centre, y_centre=y_centre, radius=radius
        )
        # Guardado de objeto
        global geometries
        ...
        self.saved_parameters_text.insert(
            tk.END,
            "Geometry_circle(name={}, x_centre={}, y_centre={}, radius={})\n".format(
                name, x_centre, y_centre, radius
            ),
        )
        self.saved_parameters_text.config(state="disabled")
    ...
    def save_geometry_ellipse(self):
        ...
        new_geometry_ellipse = Geometry_ellipse(
            name=name,
            x_centre=x_centre,
            y_centre=y_centre,
            width=width,
        ...
        self.saved_parameters_text.insert(
            tk.END,
            "Geometry_ellipse(name={}, x_centre={}, y_centre={}, width={}, height={}, beta={})\n".format(
                name, x_centre, y_centre, width, height, beta
            ),
        )
        self.saved_parameters_text.config(state="disabled")
