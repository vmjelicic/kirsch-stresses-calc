...
class BoundaryBox:
            ...
            for j in np.arange(-self.dim_y, self.dim_y, self.sep_y):
                ...
                for geometry in self.geometries:
                    if geometry.__class__.__name__ == "Geometry_circle":
                        if (
                            (i - geometry.x_centre) ** 2 + (j - geometry.y_centre) ** 2
                        ) < geometry.radius ** 2:
                        ...
                        x_tf = (
                            self.transform_x(
                                x=i - geometry.x_centre,
                                y=j - geometry.y_centre,
                                theta=geometry.beta,
                            )
                            + geometry.x_centre
                        )
                        ...
                        if (
                            ((x_tf - geometry.x_centre) ** 2)
                            / ((geometry.width / 2) ** 2)
                            + ((y_tf - geometry.y_centre) ** 2)
                            / ((geometry.height / 2) ** 2)
                        ) < 1:
                            out_of_geometry = False
    ...
    def kirsch_r(self, x_point, y_point, x_centre_geometry, y_centre_geometry):
        return np.sqrt(
            (x_point - x_centre_geometry) ** 2 + (y_point - y_centre_geometry) ** 2
        )
    ...
    def kirsch_theta(self, x_point, y_point, x_centre_geometry, y_centre_geometry):
        ...
        if (x_centre_geometry - x_point) == 0:
            ...
            return np.arctan(
                (y_point - y_centre_geometry) / (x_point - x_centre_geometry)
            )

    # Funciones de cálculo de esfuerzos inducidos (circunferencia)
    def kirsch_sigma_r(self, sx, sy, txy, r, a, theta):
        return ((sx + sy) / 2) * (1 - (a / r) ** 2) + (
            1 + 3 * (a / r) ** 4 - 4 * (a / r) ** 2
        ) * (((sx - sy) / 2) * np.cos(2 * theta) + txy * np.sin(2 * theta))

    def kirsch_sigma_theta(self, sx, sy, txy, r, a, theta):
        return ((sx + sy) / 2) * (1 + (a / r) ** 2) - (1 + 3 * (a / r) ** 4) * (
            ((sx - sy) / 2) * np.cos(2 * theta) + txy * np.sin(2 * theta)
        )

    def kirsch_tau_r_theta(self, sx, sy, txy, r, a, theta):
        return (1 - 3 * (a / r) ** 4 + 2 * (a / r) ** 2) * (
            ((sy - sx) / 2) * np.sin(2 * theta) + txy * np.cos(2 * theta)
        )
    ...
    def kirsch_ellipse_sigma_r(self, p, e0, e, C, J, K, phi, beta):
        return ((p * (e0 - e)) / (J ** 2)) * (
            ...
            + (1 - K)
            * (
                ((J / 2) * (e - e0) + C * e) * np.cos(2 * (phi + beta))
                - C * np.cos(2 * beta)
            )
        )
    ...
    def kirsch_ellipse_sigma_theta(self, p, e0, e, J, K, phi, beta, sigma_r):
        return (p / J) * (
            (1 + K) * (e ** 2 - 1)
            + 2 * (1 - K) * e0 * (e * np.cos(2 * (phi + beta)) - np.cos(2 * beta))
        ) - sigma_r

    # Cálculo de esfuerzo cortante
    def kirsch_ellipse_tau_r_theta(self, p, e0, e, C, J, K, phi, beta):
        return ((p * (e0 - e)) / (J ** 2)) * (
            (1 + K) * ((C * e) / e0) * np.sin(2 * phi)
            + (1 - K)
            * (
                e * (e0 + e) * np.sin(2 * beta)
                + e * np.sin(2 * (phi - beta))
                - ((J / 2) * (e0 + e) + e ** 2 * e0) * np.sin(2 * (phi + beta))
            )
        )
    ...
    def plot(self):
        ...
        for geometry in self.geometries:
            if geometry.__class__.__name__ == "Geometry_circle":
                x_geom_upper = [
                    x + geometry.x_centre - geometry.radius
                    for x in np.arange(0, 2 * geometry.radius + 1, 1)
                ]
                x_geom_lower = [
                    x + geometry.x_centre - geometry.radius
                    for x in np.arange(0, 2 * geometry.radius + 1, 1)
                ]
                y_geom_upper_contour = [
                    np.sqrt(geometry.radius ** 2 - (x - geometry.x_centre) ** 2)
                    + geometry.y_centre
                    for x in x_geom_upper
                ]
                y_geom_lower_contour = [
                    -np.sqrt(geometry.radius ** 2 - (x - geometry.x_centre) ** 2)
                    + geometry.y_centre
                    for x in x_geom_lower
                ]

            elif geometry.__class__.__name__ == "Geometry_ellipse":
                # Generar puntos en el sistema de referencia (x,y)
                x_geom = [
                    x + geometry.x_centre - (geometry.width / 2)
                    for x in np.arange(0, geometry.width + 1, 1)
                ]
                y_geom_u = [
                    (geometry.height / 2)
                    * np.sqrt(1 - ((x - geometry.x_centre) / (geometry.width / 2)) ** 2)
                    + geometry.y_centre
                    for x in x_geom
                ]
                y_geom_l = [
                ...
                for i in range(len(x_geom)):
                    x_geom_upper[i] = (
                        self.transform_xt(
                            x=x_geom[i] - geometry.x_centre,
                            y=y_geom_u[i] - geometry.y_centre,
                            theta=geometry.beta,
                        )
                        + geometry.x_centre
                    )
                    y_geom_upper_contour[i] = (
                        self.transform_yt(
                            x=x_geom[i] - geometry.x_centre,
                            y=y_geom_u[i] - geometry.y_centre,
                            theta=geometry.beta,
                    ...
                    x_geom_lower[i] = (
                        self.transform_xt(
                            x=x_geom[i] - geometry.x_centre,
                            y=y_geom_l[i] - geometry.y_centre,
                            theta=geometry.beta,
                        )
                        + geometry.x_centre
                    )
                    y_geom_lower_contour[i] = (
                        self.transform_yt(
                            x=x_geom[i] - geometry.x_centre,
                            y=y_geom_l[i] - geometry.y_centre,
                            theta=geometry.beta,
    ...
    def calculate_kirsch_stresses(self, P, Q, angle_P=0):
        ...
        self.sx_insitu = self.rotate_sigma_x(
            sx=self.P, sy=self.Q, txy=0, theta=-self.angle_P
        )
        self.sy_insitu = self.rotate_sigma_y(
            sx=self.P, sy=self.Q, txy=0, theta=-self.angle_P
        )
        self.txy_insitu = self.rotate_tau_xy(
            sx=self.P, sy=self.Q, txy=0, theta=-self.angle_P
        )
        ...
        for geometry in self.geometries:
            ...
            if geometry.__class__.__name__ == "Geometry_circle":
                ...
                for i in self.boundary_df.index:
                    ...
                    x_t = self.transform_x(
                        x=self.boundary_df["x"][i] - geometry.x_centre,
                        y=self.boundary_df["y"][i] - geometry.y_centre,
                        theta=self.angle_P,
                    )
                    y_t = self.transform_y(
                        x=self.boundary_df["x"][i] - geometry.x_centre,
                        y=self.boundary_df["y"][i] - geometry.y_centre,
                        theta=self.angle_P,
                    )
                    ...
                    calc_e0 = self.kirsch_ellipse_e0(
                        width=geometry.width, height=geometry.height
                    )

                    # kirsch_ellipse_b(self, width, height, x1, z1):
                    calc_b = self.kirsch_ellipse_b(
                        width=geometry.width, height=geometry.height, x1=x_t, z1=y_t
                    )

                    # kirsch_ellipse_d(self, W, H, x1, z1):
                    calc_d = self.kirsch_ellipse_d(
                        width=geometry.width, height=geometry.height, x1=x_t, z1=y_t
                    )
    ...
    def calculate_strength_factor(self, sci, mi, gsi):
        ...
        for i in self.boundary_df.index:
            ...
            calc_p_rockmass = self.hoek_brown_S1(
                s3=self.boundary_df["Q"][i], sci_rock=sci, mi_rock=mi, gsi_rockmass=gsi
            )
            # strength_factor(self, s1_induced, s1_rockmass)
            calc_strength_factor = self.strength_factor(
                s1_induced=self.boundary_df["P"][i], s1_rockmass=calc_p_rockmass
            )
